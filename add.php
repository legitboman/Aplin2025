<?php
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if (empty($title) || empty($caption) || empty($_FILES['image']['name']) || $harga === '' || $stok === '') {
            ...
            if ($image) {
                $query = $pdo->prepare("INSERT INTO posts (user_id, title, caption, image, harga, stok, category_id) VALUES (?, ?, ?, ?, ?, ?, ?)");
                $query->execute([$user['id'], $title, $caption, $image, $harga, $stok, $category_id]);
